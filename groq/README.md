...
- How to prompt [`Groq`](https://groq.com) to generate out-of-the-box functional code using a [`Groq API key`](https://console.groq.com/keys);
- The folder `isValidEmailPhoneOrSocialMediaUrl` used as example below is just a way of testing the main **Python code**, a simple template to generate a **JavaScript function**, save it to a file and check if it works with some kind of test or validation script.
...
git clone \
  --depth 1 \
  --no-checkout https://github.com/tomasfn87/llm-dev ./groq \
  && cd groq \
  && git sparse-checkout set groq \
  && git checkout \
  && mv groq/* . \
  && rm -rf {groq,.git}
```
...
- Rename file `env_template` to `.env`:
  - Insert your [API key from Groq Console](https://console.groq.com/keys).
...
```shell
./generate_llama_code_and_save_to_file.sh \
